...
inputpass()
{
	dialog --backtitle "$_backtitle" --title "$_pass_title" --inputbox "$_pass_body" 0 0 "" 2>${ANSWER}
	qst=$?
	case $qst in
		0) _userpass=$(cat ${ANSWER})
			;;
	esac
}
select_type_pass()
{
	dialog --default-item 2 --backtitle "$_backtitle" --title "$_select_type_pass_title" \
    ...
    case $(cat ${ANSWER}) in
        "1") echo "_type_mount=\"1\"" >> $filedir/options.conf
             ;;
        "2") inputpass
			echo "_type_mount=\"2\"" >> $filedir/options.conf
			[[ ${_userpass[*]} != "" ]] && echo "szPassword=\"${_userpass[*]}\"" >> $filedir/options.conf
             ;;
        "3") echo "_type_mount=\"3\"" >> $filedir/options.conf
             ;;
     esac
}
select_language
dialog --backtitle "$_backtitle" --title "$_info_install_title" --msgbox "$_info_install_body" 0 0
dialog --backtitle "$_backtitle" --title "$_info_pass_type_title" --msgbox "$_info_pass_type_body" 0 0
select_type_pass
rm -rf $ANSWER
clear
exit 0
